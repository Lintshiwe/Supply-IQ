...
function useDemoMutation<TData>(
  ...
  const mutate = useCallback(
    (data: TData, opts?: { onSuccess?: () => void; onError?: (e: Error) => void }) => {
      // Authenticated mode: mutations go through server functions
      // Demo mode: mutations go through DemoStore
      if (isAuthenticated) {
        setIsLoading(true);
        // Server function calls would go here. For now, delegate to demo store if available
        if (isDemo && demoStore) {
          try {
    ...
    [isDemo, isAuthenticated, demoStore, handler, bumpVersion],
  );
